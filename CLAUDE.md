...
- Env: `.envrc` exports `SWTCG_CARD_DB_CS="sqlite://cards.db"` (required; read via aero `#env` in `resources/config.edn`, validated by malli in `stevetrip.swtcg.deck-builder.config`). Load it (direnv) before starting a REPL.
- Dev REPL: `clj -M:dev`, then use integrant-repl in `dev/user.clj`: `(go)` / `(reset)`. Server starts on port 3000 (`stevetrip.swtcg.deck-builder.system`). Migrations run as part of the system (`::migrations`).
...
- **Wiring**: `stevetrip.swtcg.deck-builder.system` defines the integrant system map: config → `::connection` → `::db` → `::app` (reitit ring handler) → `::server` (jetty). `stevetrip.swtcg.deck-builder.core/-main` is a separate, simpler entry point that runs `routes/app` directly.
- **DB abstraction**: `stevetrip.swtcg.deck-builder.db.db` parses the connection string URI (`sqlite://cards.db`) into a map and dispatches on `:scheme` to create a database implementation (`db/sqlite.clj` for SQLite, `db/memory.clj` for in-memory). Handlers/services depend on the db protocol rather than SQL directly.
- **SQL**: SQLite impl uses HugSQL; queries live in `src/stevetrip/swtcg/deck_builder/db/sql/{cards,decks}.sql` and functions are generated with `def-db-fns` (hence the `declare`s in `sqlite.clj`). Results use `as-unqualified-kebab-maps`, so DB rows have kebab-case keys. Schema comes from `resources/migrations` (migratus). `cards.db` / `test.db` are local SQLite files (with WAL/SHM side files).
- **IDs**: `card_id` is a deterministic 12-hex-char SHA-1 of `image_file` (`stevetrip.swtcg.deck-builder.tools.load-cards/card-id`), so it survives a `cards.db` rebuild. `deck_id` is a random UUID string generated in `sqlite.clj`. Both are TEXT everywhere (schema, routes, malli).
- **Web layer** (`stevetrip.swtcg.deck-builder.web.*`): `routes` (reitit + swagger/coercion + CORS), `handlers`, `schema` (malli request/response schemas), `middleware`, `error` (error mapping using `resources/http-errors.edn`).
- **Deck validation**: `stevetrip.swtcg.deck-builder.validation.rules` is a Clara Rules rulebase (facts `Deck`, `Card`, `Violation`; e.g. 60-card size, max 4 copies), driven by `stevetrip.swtcg.deck-builder.validation.service`. `stevetrip.swtcg.deck-builder.services.deck_service` orchestrates deck operations.
